...
const cart=await Cart.find({userId:userId}).populate({
    path:'productId',
    select:'imageUrl title pharamcy rating ratingCount',
    populate:{
        path:'pharmacy',
        selct:'time coords'
    }
...
if(cartItem){
    ...
    if(cartItem.quantity>1){
        ...
        await cartItem.save();
        res.status(200).json({status:true,message:"Product Quality Successfully decremented"})
    }else{
